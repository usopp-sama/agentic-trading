...
todos:
  ...
  - id: tool-phase2
    ...
    status: pending
  - id: tool-phase3
    content: "Build signals and alerts: combine tech + fundamental signals, position sizing (Kelly), Telegram/email notifications. Done in repo: signal combination via CIO, fractional-Kelly sizing, Telegram alerts + one-tap trade approvals (inline buttons -> audited approval path). Email (SMTP) channel built: notify() funnel + EmailService deliver alerts, the daily digest, and approval requests over TLS; the preferred channel for the paper run."
    status: in_progress
