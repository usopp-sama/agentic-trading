...
var/knowledge/                   # CLEANED, chunked notes (ingested; reliability 95)
...
2. **Phase 2:** add `scripts/ingest_knowledge.py` (parse + clean + structure-chunk
   + front-matter metadata) so you can drop PDFs/reports into `library/` and
   auto-generate clean notes. New deps: `pypdf`/`pdfplumber`, `trafilatura`.
